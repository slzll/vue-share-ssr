...
(function(window, document, undefined) {
  ...
  function createIcons(elem, data) {
    ...
    each(isPrepend ? sites.reverse() : sites, function(name) {
      ...
      var link = data.initialized
        ...
        : createElementByString('<a class="social-share-icon icon-' + name + '"></a>');
  ...
  function createWechat(elem, data) {
    ...
    if (data.url) {
      QRCode.toDataURL(data.url, { margin: 0, width: data.wechatQrcodeSize }).then(function(url) {
        qrcode[0].innerHTML = "<img src=" + url + " />";
